...
exports.showData = async (req, res) => {
	...
	if (data.length === 0)
		return res
			...
			.json({ info: 'get data in table Category', data: 'don`t have data' });

	return res
		.status(codes.ACCEPTED)
		.json({ info: 'get data in table Category', data });
};
...
exports.showOne = async (req, res) => {
	...
	if (!data)
		return res
			.status(codes.ACCEPTED)
			.json({
				info: 'get data in table Category',
				data: 'don`t have this data',
			});
...
exports.update = async (req, res) => {
	...
	if (hasUpdated[0] == 0)
		return res
			.status(codes.ACCEPTED)
			.json({
				info: 'updated data in table Category',
				old,
				moreInfo: 'data not found',
			});

	return res
		.status(codes.ACCEPTED)
		.json({ info: 'updated data in table Category', old });
};
