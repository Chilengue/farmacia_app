...
class Categoria {
	...
	isVailid(data) {
		...
		for (const da in data) {
			if (da == 'nameCategory') {
				if (!valid.isLength(data[da], { min: 4, max: 30 }))
					this.Errors.push(
						'nome da categoria deve conter no minímo 5 caracteres!!!',
					);
			...
			if (da == 'description') {
				if (valid.isEmpty(data[da]))
					this.Errors.push('descrição da categoria é obrigatória!!!');
			}
		}
